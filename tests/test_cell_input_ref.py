...
def test_input_ref_property_rejects_values_and_keeps_old_hold(value):
    ...
    cell = Cell("bytes", checksum=checksum)
    with pytest.raises(AttributeError, match="source or checksum"):
...
def test_checksum_input_forms():
    ...
    by_keyword = Cell(checksum=checksum, celltype="plain")
    by_property = Cell(celltype="plain")
    by_property.checksum = checksum
    ...
    for cell in (by_keyword, by_property, by_set_checksum):
        assert isinstance(cell.checksum, Checksum)
        assert cell.checksum == checksum
...
def test_reference_inputs_are_accepted():
    ...
    upstream = Cell(checksum=checksum)
    assert Cell(checksum=None).source is None
    assert Cell(source=expression).source is expression
    assert Cell(source=upstream).source is upstream
...
def test_set_hex_string_is_a_value_not_a_checksum():
    ...
    cell.set(text)
    assert cell.checksum != Checksum(text)
...
def test_set_value_replaces_and_releases_previous_hold():
    ...
    assert cell.run() == 2
    cell.set(None)
    assert cell.checksum == Buffer(None, "int").get_checksum()
    assert cell.value is None


def test_failed_set_leaves_input_unchanged():
    checksum = Buffer(b"failed set").get_checksum()
    cell = Cell("int", checksum=checksum, input_celltype="bytes")
    with pytest.raises(ValueError):
        cell.set("not-an-integer")
    assert cell._input_ref == checksum
    assert _count(checksum) == 1
    cell._release_refholds()
